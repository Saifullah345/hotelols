...
const ROLES = ['all', 'customer', 'hotel_admin', 'staff', 'super_admin'] as const
type RoleFilter = typeof ROLES[number]

const ROLE_LABELS: Record<string, string> = {
  all: 'All', customer: 'Customer', hotel_admin: 'Hotel Admin',
...
export default async function UsersPage({
  ...
  const { page, q, role: roleParam } = await searchParams
  const activeRole: RoleFilter = (ROLES as readonly string[]).includes(roleParam ?? '') ? roleParam as RoleFilter : 'all'
  ...
  const users = (allUsers ?? []).filter(user => {
    ...
    if (query && !user.full_name?.toLowerCase().includes(query) && !user.email?.toLowerCase().includes(query)) return false
    return true
  })
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {ROLES.map(r => {
            const count = r === 'all' ? (allUsers?.length ?? 0) : (allUsers ?? []).filter(u => u.role === r).length
            const href  = `/super-admin/users?${r !== 'all' ? `role=${r}` : ''}${q ? `&q=${encodeURIComponent(q)}` : ''}`
            return (
              <Link
                key={r}
                href={href}
